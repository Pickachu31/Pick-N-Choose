...
flightAPI.getAiportTravelDestination = (req, res, next)=>{ 
  ...
  unirestRequest.end( (response)=>{ 
    ...
    // console.log('here in flight APi get travel destinations', response.body.Places)
    return next(); 
  }); 
} 
...
flightAPI.getMinimumFlightPrices = (req, res, next)=>{ 
  ...
  const mappedData = res.locals.places.map( place =>{ 
    ...
    return new Promise( (resolve, reject)=>{ 
      ...
      const unirestRequest = unirest("GET", `https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/browsequotes/v1.0/US/USD/en-US/LAX-sky/${place.PlaceId}/${req.body.departureDate}`);

      unirestRequest.query({
        "inboundpartialdate": req.body.returnDate
      });
      unirestRequest.headers({ "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com", "x-rapidapi-key": "2b3aee7f7emsh9a3043bf4a78f25p186a4ejsn07fdb9fc52ed" });
      ...
      unirestRequest.end( (response) =>{ 
        ...
        console.log('here in get minimum flight prices', response.body.Agents)
         ...
         console.log('here in request end', minPrices)
